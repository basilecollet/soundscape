        ...
        @foreach($availableSettings as $page => $pageSettings)
            <div class="bg-white dark:bg-zinc-900 shadow rounded-lg border border-gray-200 dark:border-zinc-700">
                <div class="px-6 py-4 border-b border-gray-200 dark:border-zinc-700">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-zinc-100 capitalize">{{ $page }} Page</h3>
                </div>

                <div class="px-6 py-4 space-y-4">
                    @if(empty($pageSettings))
                        <p class="text-sm text-gray-500 dark:text-zinc-400">All sections on this page are always enabled and cannot be disabled.</p>
                    @else
