<div>
    ...
    <div class="mb-8">
        ...
        <div class="flex flex-col lg:flex-row gap-4 p-4 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-zinc-900 dark:to-zinc-800 rounded-xl border border-gray-200 dark:border-zinc-700">
            <flux:field class="flex-1">
                <flux:label class="text-gray-700 dark:text-zinc-300 font-medium">{{ __('admin.content.filter.page_label') }}</flux:label>
                <flux:select wire:model.live="selectedPage" placeholder="{{ __('admin.content.filter.all_pages') }}">
                    <flux:select.option value="all">{{ __('admin.content.filter.all_pages') }}</flux:select.option>
            ...
            <flux:field class="flex-1">
                <flux:label class="text-gray-700 dark:text-zinc-300 font-medium">{{ __('admin.content.filter.search_label') }}</flux:label>
                <flux:input wire:model.live="search" placeholder="{{ __('admin.content.filter.search_placeholder') }}" />
            </flux:field>
        ...
        @if($contents->count() > 0)
            <div class="flex items-center justify-between mt-4 px-1">
                <div class="text-sm text-gray-600 dark:text-zinc-400">
                    {{ __('admin.content.results.showing') }} <span class="font-medium">{{ $contents->count() }}</span> {{ $contents->count() !== 1 ? __('admin.content.results.contents') : __('admin.content.results.content') }}
                    @if($search)
                        {{ __('admin.content.results.for') }} "<span class="font-medium">{{ $search }}</span>"
                    @endif
                    @if($selectedPage !== 'all')
                        {{ __('admin.content.on') }} <span class="font-medium">{{ $selectedPage }}</span> {{ __('admin.content.page') }}
                    @endif
                </div>

                @if($contents->count() > 5)
                    <div class="text-xs text-gray-500 dark:text-zinc-500">
                        {{ __('admin.content.results.use_ctrlf') }}
                    </div>
                @endif
            </div>
        @endif
                            ...
                            <td class="px-6 py-4 text-sm text-gray-600 dark:text-zinc-400">
                                <div class="max-w-xs">
                                    @if($content->content)
                                        ...
                                        <p class="text-xs text-gray-400 dark:text-zinc-500 mt-1">{{ strlen($content->content) }} {{ __('admin.content.form.content.characters') }}</p>
                                    @else
                                        <span class="italic text-gray-400 dark:text-zinc-500">{{ __('admin.content.empty_content') }}</span>
                                    @endif
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm">
                                <div class="flex items-center space-x-2">
                                    <flux:button
                                        ...
                                        wire:navigate
                                        aria-label="{{ __('ui.common.edit') }} {{ $content->key }} content"
                                    >
                                    ...
                                    @if($content->content)
                                        ...
                                        >
                                            ...
                                            <svg x-show="copied" x-cloak class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
    ...
    @else
        <div class="text-center py-12">
            ...
            <h3 class="text-lg font-medium text-gray-900 dark:text-zinc-100 mb-2">{{ __('admin.content.empty_state.title') }}</h3>
            <p class="text-gray-500 dark:text-zinc-400">{{ __('admin.content.empty_state.description') }}</p>
        </div>
    @endif

    @if(!empty($missingKeys))
        <div class="mt-8">
            <h3 class="text-lg font-semibold text-gray-900 dark:text-zinc-100 mb-4">{{ __('admin.content.missing_keys.title') }}</h3>
            <div class="grid gap-4">
                @foreach($missingKeys as $page => $keys)
                    <div class="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-6">
                        <h4 class="text-base font-medium text-gray-900 dark:text-zinc-100 mb-3">{{ ucfirst($page) }} {{ __('admin.content.page') }}</h4>
                        <div class="space-y-3">
                            @foreach($keys as $key)
